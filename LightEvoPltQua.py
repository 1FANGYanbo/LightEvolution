...
inDir = "./LightEvolutionQua/Q10000" + "zTot100" + "/"
inFileName = inDir + "EAllQ10000" + "data.csv"
...
for q in pltQVals:
    tValsTmp = [dz * q] * L
    ax1.plot(sites[int(abs(nStart) - truncation / 2): int(abs(nStart) + truncation / 2)], tValsTmp[:truncation],
             strVec2ComplexVecAbs(inData.iloc[q, ])[
             int(abs(nStart) - truncation / 2): int(abs(nStart) + truncation / 2)], color="black")
...
for q in pltQRedVals:
    ...
    ax1.plot(sites[int(abs(nStart) - truncation / 2): int(abs(nStart) + truncation / 2)], tValsTmp[:truncation],
             strVec2ComplexVecAbs(inData.iloc[q, ])[
             int(abs(nStart) - truncation / 2): int(abs(nStart) + truncation / 2)], color="red")
...
# ax3.set_yticks([-0.75, -0.5, -0.25, 0, 0.25, 0.5, 0.75])
...
fig.suptitle("Light Evolution Quadratic", fontsize=ftSize)
plt.savefig(inDir + "EAllQuaQ" + str(Q) + "C0" + str(C0) + ".png")
plt.close()
